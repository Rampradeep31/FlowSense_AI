...
import { LuLock, LuUser, LuShieldAlert } from 'react-icons/lu';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.detail) {
        ...
        setError("Network error. Please make sure the FastAPI server is running.");
      }
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center py-12 px-6">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-xl">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="font-outfit font-bold text-2xl text-slate-900">Sign In to FlowSense AI</h2>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            >
              {submitting ? "Verifying Credentials..." : "Access System"}
            </button>
          </div>
        </form>
